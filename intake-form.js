(() => {
  ...
  async function preparePhoto(file) {
    ...
    if (file.size > 20 * 1024 * 1024) throw new Error(`${file.name}: please choose a photo smaller than 20 MB.`);
    let bitmap;
    ...
    try {
      ...
      if (!blob || blob.size > 600000) throw new Error(`${file.name}: please choose a smaller or simpler photo.`);
  ...
  async function addPhotos(selected) {
    ...
    if (selectedFiles.length + files.length > 30) errors.push('You can send up to 30 photos. Extra photos were not added.');
  ...
  void api('/api/intake/config').then(config => {
    available = config.enabled; $('submit-button').disabled = !available;
