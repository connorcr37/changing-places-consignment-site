...
(async () => {
  ...
  try {
    ...
    for (const width of [390, 1280]) {
      const notesForm = await setup(width);
      const notes = notesForm.page.locator('#customer-notes');
      const counter = notesForm.page.locator('#notes-count');
      const normalCounterColor = await counter.evaluate(element => getComputedStyle(element).color);
      const normalBorderColor = await notes.evaluate(element => getComputedStyle(element).borderColor);
      assert.equal(await counter.textContent(), '0 / 2,000 characters');
      assert.equal(await counter.isVisible(), false, 'Keep the counter quiet until notes are focused or populated');
      await notes.focus();
      assert.equal(await counter.isVisible(), true);
      await notes.press('Tab');
      assert.equal(await counter.isVisible(), false, 'An empty field hides its counter again on blur');
      const exampleToggle = notesForm.page.getByRole('button', { name: 'See an example', exact: true });
      const example = notesForm.page.locator('#notes-example');
      ...
      assert.equal(await example.isVisible(), false);
      assert.equal(notesForm.calls.starts.length, 0, 'The example disclosure must not submit the form');
      await notes.fill('a'.repeat(1999));
      assert.equal(await counter.textContent(), '1,999 / 2,000 characters');
      await notes.press('Tab');
      assert.equal(await counter.isVisible(), true, 'Keep the counter visible after populated notes lose focus');
      await notes.press('a');
      assert.equal(await counter.textContent(), '2,000 / 2,000 characters');
      assert.equal(await notes.getAttribute('aria-invalid'), 'false');
      const normalOutlineColor = await notes.evaluate(element => getComputedStyle(element).outlineColor);
      await notes.press('b');
      assert.equal(await notes.inputValue(), 'a'.repeat(2000) + 'b', 'Typing beyond the limit must preserve every character');
      assert.equal(await counter.textContent(), '2,001 / 2,000 characters');
      assert.equal(await notes.getAttribute('aria-invalid'), 'true');
      const overLimitColor = await counter.evaluate(element => getComputedStyle(element).color);
      assert.notEqual(overLimitColor, normalCounterColor, 'The counter must highlight over-limit notes');
      assert.equal(await notes.evaluate(element => getComputedStyle(element).borderColor), overLimitColor, 'The textarea border must match the highlighted counter');
      assert.equal(await notes.evaluate(element => getComputedStyle(element).outlineColor), overLimitColor, 'The focused textarea outline must turn red too');
      assert.notEqual(await notes.evaluate(element => getComputedStyle(element).outlineStyle), 'none', 'Keep a visible keyboard focus indicator');
      await notesForm.page.locator('#submit-button').click();
      assert.equal(notesForm.calls.starts.length, 0, 'Over-limit notes must block submission');
      assert.equal(await notes.evaluate(element => element === document.activeElement), true);
      assert.equal(await notesForm.page.locator('[role="alert"]:visible').count(), 0, 'The counter and border provide feedback without a separate error box');
      await notesForm.page.context().grantPermissions(['clipboard-read', 'clipboard-write']);
      const pastedNotes = 'Details\n'.repeat(625) + 'x';
      await notesForm.page.evaluate(text => navigator.clipboard.writeText(text), pastedNotes);
      await notes.press('ControlOrMeta+A');
      await notes.press('ControlOrMeta+V');
      await notesForm.page.waitForFunction(() => document.getElementById('notes-count').textContent === '5,001 / 2,000 characters');
      assert.equal(await notes.inputValue(), pastedNotes, 'A long paste must remain intact, including line breaks');
      assert.equal(await notes.getAttribute('aria-invalid'), 'true');
      assert.equal(await notesForm.page.evaluate(() => document.documentElement.scrollWidth > innerWidth), false);
      await notes.fill('');
      assert.equal(await counter.textContent(), '0 / 2,000 characters');
      assert.equal(await notes.getAttribute('aria-invalid'), 'false');
      assert.equal(await counter.evaluate(element => getComputedStyle(element).color), normalCounterColor);
      assert.equal(await notes.evaluate(element => getComputedStyle(element).borderColor), normalBorderColor);
      assert.equal(await notes.evaluate(element => getComputedStyle(element).outlineColor), normalOutlineColor);
      await notes.press('Tab');
      assert.equal(await counter.isVisible(), false, 'Clearing and leaving notes restores the quiet state');
      await notes.fill('a'.repeat(2000));
      await notesForm.page.locator('#photo-input').setInputFiles(uploadFile('sofa.jpg'));
      await notesForm.page.waitForFunction(() => document.getElementById('photo-count').textContent === '1 of 30 photos');
      await notesForm.page.locator('#submit-button').click();
      await notesForm.page.locator('#intake-success').waitFor({ state: 'visible' });
      assert.equal(notesForm.calls.starts[0].notes, 'a'.repeat(2000), 'Exactly 2,000 characters must submit unchanged');
      await notesForm.page.close();
    }
    console.log('Notes checks passed on mobile and desktop: live counts, 2,000-character boundary, intact long paste, counter and border feedback without an error box, blocked over-limit submission, and recovery.');
