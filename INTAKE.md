...
The email starts with the consignor name, phone, email and notes, including in realistic synthetic tests. Each item has one brief sentence about visible cleanliness/condition, meaningful flaws, or category mismatch. Identifiable submitted objects outside the accepted categories (for example, a water bottle) still count as items and receive an Unlikely fit rating and explanation beside their photo, including in mixed batches. Only photos with no identifiable submitted object return an empty item list. The analysis and email omit pricing, follow-up questions, and reply drafts.

The contact header shows the phone number once with plain **Call** and **Text** buttons, no emojis or separator dots. The number and Call button use `tel:`. Gmail strips direct `sms:` email links, so Text uses an HTTPS link to `/text-consignor`; that page requires one tap on **Open text message** to launch a blank SMS. The phone number is encoded only in the URL fragment (not sent to the server), validated locally, and removed from browser history immediately. No contact data is stored by that page, and its CSP blocks analytics and network connections. The email address appears below as a plain, pre-addressed `mailto:` link with no subject or body parameters; the submission timestamp follows. Missing phone details omit the call/text row, and missing email displays “No email provided”.
...
The browser prepares a detailed JPEG for the AI (up to 1600 pixels / 600 KB) and an email copy (up to 1200 pixels / 100 KB). Both copies adapt JPEG quality and dimensions down to a 240-pixel longest edge if necessary to meet their separate byte budgets. A failed browser conversion has a preparation/re-save message; only source files over 20 MB receive the source-size error. Sequential uploads avoid one large request. Thirty email copies fit within the email service's size limit. Resizing strips original EXIF metadata. Public validation rules and photo limits live in `intake-shared.js`, shared by the browser and Worker. Storage keys and bounded body reading live in `worker/intake-utils.mjs`. No public photo URLs exist.
