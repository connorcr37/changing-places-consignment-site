...
export async function recoverIntake(env) {
  ...
  const expired = await query(env, "SELECT * FROM intake_submissions WHERE state='deleting' OR (state='uploading' AND created_at<?) OR (submitted_at IS NOT NULL AND submitted_at<?) LIMIT 20", now() - 86400, now() - 30 * 86400).all();
